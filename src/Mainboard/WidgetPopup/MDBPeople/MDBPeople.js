...
import MDBPeopleInfo from '../../Widgets/MDBPeopleInfo/MDBPeopleInfo'

import './../WidgetStyle.css'

class MDBPeople extends Component {
	state = {
		language: 'fr-FR',
		texte: '',
		timer: 10
	...
	createWidget = () => {
		...
		let apiUrl = 'https://api.themoviedb.org/3/search/person?api_key=' + apiKey + '&language=' + this.state.language + '&query=' + this.state.texte
			...
			<div className='widgetBox'>
				...
				<div className='infoPlaceholder'>
					<div className='topPart'>
						<p className='titleStyle'>Find actor by name:</p>
						<div className="widgetTooltip">i
							<span className="widgetTooltiptext">Show the series with the maching name</span>
						</div>
					</div>
					<p className='widgetSubtitle'>Actor name : </p>
					<input
						type='text'
						placeholder='Example : Natalie Portman'
