...
class LFMTracksInfo extends Component {
	constructor ( props ) {
		super(props);
		this.state = {
		...
		this.urlPart = 'https://ws.audioscrobbler.com/2.0/?method=tag.gettoptracks&tag='
    ...
    componentDidMount() {
		...
		FetchData(this.apiUrl).then(res => {
			this.setState({ apiRes: res.tracks.track })
		})
	}
	...
	render() {
		...
		let tracks = (
			<div className='movieFirstDiv'>
				{this.state.apiRes.map(( track, index ) => {
					return (
						<div className='moviemainView' key={'tracks' + index}>
							<img
							...
							<div className='movietextZone'>
								...
								<div className='moviedateZone'>
									<a className='moviedateText' href={track.url} target='_blank'>See more ...</a>
								</div>
							</div>
						</div>
					)
			...
			<div className='ytci'>
				<div className='infoPlaceholder'>
					<div className='youtubeChannelTop'>
						...
						<h2 className='youtubeChannelInfoTitle'>Top Tracks</h2>
