...
class MDBSearchInfo extends Component {
	constructor ( props ) {
		super(props);
		this.state = {
		...
		this.urlPart = 'https://api.themoviedb.org/3/search/movie?api_key='
	...
	render() {
		...
		let movies = (
			<div className='movieFirstDiv'>
				{this.state.apiRes.results.map(( film, index ) => {
					return (
						<div className='moviemainView' key={film.id}>
							<img
							...
							<div className='movietextZone'>
								...
								<div className='moviedateZone'>
									<span className='moviedateText'>Released : {film.release_date}</span>
								</div>
							</div>
						</div>
					)
			...
			<div className='ytci'>
				<div className='infoPlaceholder'>
					<div className='youtubeChannelTop'>
						...
						<h2 className='youtubeChannelInfoTitle'>Movies :</h2>
