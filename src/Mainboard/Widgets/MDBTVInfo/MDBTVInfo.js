...
class MDBTVInfo extends Component {
	constructor ( props ) {
		super(props);
		this.state = {
		...
		this.urlPart = 'https://api.themoviedb.org/3/search/people?api_key='
	...
	render() {
		...
		let movies = (
			<div className='movieFirstDiv'>
				{this.state.apiRes.results.map(( tv, index ) => {
					return (
						<div className='moviemainView' key={tv.id}>
							<img
							...
							<div className='movietextZone'>
								...
								<div className='moviedateZone'>
									<span className='moviedateText'>Released : {tv.first_air_date}</span>
								</div>
							</div>
						</div>
					)
			...
			<div className='ytci'>
				<div className='infoPlaceholder'>
					<div className='youtubeChannelTop'>
						...
						<h2 className='youtubeChannelInfoTitle'>Series :</h2>
