...
class STAppNewsInfo extends Component {
	constructor ( props ) {
		super(props);
		this.state = {
			...
			tmpNewCount: this.props.count,
			tmpNewTimer: this.props.timer,
			seconds: 0
		}
		this.urlPart = 'https://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid='
		this.apiUrl = this.urlPart + this.props.appId + '&count=' + this.props.count + '&maxlength=300&format=json'
    ...
    componentDidMount() {
		...
		FetchData(this.apiUrl).then(res => {
			this.setState({ apiRes: res.appnews.newsitems})
		});
	}
	...
	countChangeHandler = ( event ) => {
		this.setState({ tmpNewCount: event.target.value })
	}
	...
	confirmChangesHandler() {
		let tmpUrl = ' https://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid=' + this.state.tmpNewAppId + '&count=' + this.state.tmpNewCount + '&maxlength=300&format=json'
		...
		let allNews = (
				<div className='allNewsContainer'>
					{this.state.apiRes.map(( news, index ) => {
						return (
							<div className='steamAppNewsContainer' key={news.gid}>
								<h3>{news.title}</h3>
								<div className='' dangerouslySetInnerHTML={{__html:news.contents}}>
								</div>
