...
class LFMAlbumInfo extends Component {
	constructor ( props ) {
		super(props);
		this.state = {
		...
		this.urlPart = 'https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag='
    ...
    componentDidMount() {
		...
		FetchData(this.apiUrl).then(res => {
			this.setState({ apiRes: res.albums.album })
		})
	}
						...
						<div className='moviemainView' key={'album' + index}>
							...
							<div className='movietextZone'>
								<div className='movieheaderZone'>
									<a className='movietitleText' href={album.url} target='_blank'>
										{album.name}
									</a>
								</div>
								...
								<div className='moviedateZone'>
									<span className='moviedateText'>Rank : {album["@attr"].rank}</span>
								</div>
							</div>
						</div>
					)
			...
			<div className='ytci'>
				<div className='infoPlaceholder'>
					<div className='youtubeChannelTop'>
						...
						<h2 className='youtubeChannelInfoTitle'>{this.state.tmpNewGenre} Top Albums</h2>
